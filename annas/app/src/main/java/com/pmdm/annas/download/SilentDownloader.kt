...
class SilentDownloader @Inject constructor(
    ...
    private val directExtensions =
        setOf(".pdf", ".epub", ".mobi", ".azw3", ".zip")
                    ...
                    val js = """
                        (function() {
                            function bypassAndExtract() {
                                ...
                                const urlSpans =
                                    document.querySelectorAll('span.break-all');
    ...
    fun getMime(url: String) = when {
        ...
        url.contains(".epub", true)
            -> "application/epub+zip"

        url.contains(".mobi", true)
            -> "application/x-mobipocket-ebook"

        url.contains(".azw3", true)
            -> "application/x-mobipocket-ebook"
    ...
    private fun isDirect(url: String): Boolean {
        ...
        if (
            low.contains("slow_download")
            && low.contains("annas-archive.org")
        ) return false

        return directExtensions.any {
            low.endsWith(it) || low.contains("$it?")
        } || low.contains("get.php") ||
                low.contains("libgen") ||
                low.contains("/get/") ||
                (
